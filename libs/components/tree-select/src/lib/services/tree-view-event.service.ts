...
export class TreeViewEventService {
  ...
  private updateCheckedValueOfItem(itemToCheck: any, items: any[]): void {
    const item = items.filter((value) => value.id === itemToCheck.id)[0];
  ...
  private checkAllChildItems(item: any, treeViewName: string) {
    return new Promise((resolve, reject) => {
      ...
      if (item.items) {
        item.items.forEach((itemChild: any) => {
          ...
          return this.checkAllChildItems(itemChild, treeViewName).then(
            resolve,
            reject
          );
        });
    ...
    return new Promise((resolve, reject) => {
      items.forEach((itemChild) => {
        if (itemChild.items && itemChild.items.length > 0) {
          ...
          if (itemChild.items.filter((e: any) => !e.checked).length !== 0) {
  ...
  private addCheckedItemsToCheckItemList(
    treeView: any[],
    treeViewName: string
  ): any {
    return new Promise((resolve, reject) => {
  ...
  public checkChanged(item: any, treeViewName: string, persistedName: string) {
    ...
    this.checkAllChildItems(item, treeViewName).then((r) => {
      ...
      const treeView = this.dataService.getTreeViewItemsByName(treeViewName);
      // Check or uncheck each parent of treeView
      this.checkParentItems(treeView).then((response: any) => {
        this.addCheckedItemsToCheckItemList(treeView, treeViewName).then(
          (items: any) => {
            ...
            if (
              ...
              typeof this.dataService.getCallBackByName(treeView) === 'function'
            ) {
              this.dataService.getCallBackByName(treeView)();
            }
