...
export class IonicSelectableComponent implements ControlValueAccessor, OnInit, DoCheck {
        ...
        this._groups.forEach((group) => {
          ...
          if (items.length) {
            groups.push({
              value: group.value,
              text: group.text,
              items: uniqBy(items, this.itemTextField)
            });
          }
        });
  ...
  _positionAddItemTemplate() {
    ...
    setTimeout(() => {
      const footer = this._modalComponent._element.nativeElement.querySelector(
        '.ionic-selectable-add-item-template ion-footer',
      );
  ...
  private _setHasPlaceholder() {
    this._hasPlaceholder =
      !this.hasValue() && (!this._isNullOrWhiteSpace(this.placeholder) || this.placeholderTemplate) ? true : false;
  }
  ...
  /**
   * A function optionally passed into the NgForOf directive
   * to customize how NgForOf uniquely identifies items in an iterable.
   * @param index
   * @param item
   * @returns
   */
  trackByFuntion(index: number, item: any) {
    return item[this.itemValueField]
  }
}
