...
export class TreeViewService {
  ...
  public collapseItem(item: any): any {
  ...
  public anyChildChecked(items: any[], result = false): boolean {
    if (!items || (items && items.length === 0)) {
      return false;
    }
  ...
  private hasAnyChildChecked(items: any[]): boolean {
    items.forEach((itemChild, key) => {
      if (itemChild.items && itemChild.items.length > 0) {
  ...
  private verifyIfItemIsChecked(itemID: number, treeViewName: string) {
    ...
    return (
      this.itemsChecked[treeViewName].filter((e: any) => e.id === itemID)
        .length > 0
    );
  }
  ...
  public readOnLocalStorageCheckItems(
    ...
    const jsonObjectItems = window.localStorage.getItem(persistedName);
    if (jsonObjectItems) {
      const objectItems: any = JSON.parse(jsonObjectItems);
  ...
  public loadAlreadyCheckedItems(
    ...
    return this.itemsChecked[treeViewName].map((e: any) => e.id);
  }
